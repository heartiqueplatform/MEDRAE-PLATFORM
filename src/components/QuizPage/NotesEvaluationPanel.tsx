...
export function NotesEvaluationPanel(props: NotesEvaluationPanelProps) {
    ...
    // Helper: Shared Sync Logic to reduce code repetition
    const syncStatus = async (isUnderstood: boolean, isNotUnderstood: boolean, customAttempts?: number) => {
        if (!userId) return;
            ...
            <div className="w-full mt-2 bg-white dark:bg-gray-900/50 border-0 rounded-xl overflow-hidden shadow-sm transition-all">
                ...
                <div className="p-1">
                    <textarea
                        ...
                        className="w-full min-h-[160px] p-4 text-sm bg-transparent border-0 focus:ring-0 resize-none text-gray-800 dark:text-gray-200 placeholder-gray-400 custom-scrollbar"
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 p-3 bg-gray-50/30 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <button
                            ...
                            className={`group relative p-2 rounded-lg transition-all ${understood[q.id] ? 'bg-green-100 text-green-600 dark:bg-green-900/30' : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                        >
                            ...
                            <Tooltip text="I understand this" />
                        </button>
                        ...
                        <button
                            ...
                            className={`group relative p-2 rounded-lg transition-all ${notUnderstood[q.id] ? 'bg-red-100 text-red-600 dark:bg-red-900/30' : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                                <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12ZM12 8.25a.75.75 0 0 1 .75.75v3.75a.75.75 0 0 1-1.5 0V9a.75.75 0 0 1 .75-.75Zm0 8.25a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Z" clipRule="evenodd" />
                            </svg>
                            <Tooltip text="Need more help" />
                        </button>
                        ...
                        <button
                            ...
                            className="group relative p-2 rounded-lg text-gray-400 hover:bg-blue-50 hover:text-blue-500 dark:hover:bg-blue-900/20 transition-all"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-6 h-6">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99" />
                            </svg>
                            <Tooltip text="Log another attempt" />
                        </button>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <button
                            ...
                            className={`group relative p-2 rounded-lg transition-all ${helpOthersDisabled[q.id] ? 'text-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'text-gray-400 hover:bg-blue-50 hover:text-blue-500 dark:hover:bg-blue-900/20'}`}
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-6 h-6">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 0 0 3.741-.479 3 3 0 0 0-4.682-2.72m.94 3.198.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0 1 12 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 0 1 6 18.719m12 0a5.971 5.971 0 0 0-.941-3.197m0 0A5.995 5.995 0 0 0 12 12.75a5.995 5.995 0 0 0-5.058 2.772m0 0a3 3 0 0 0-4.681 2.72 8.986 8.986 0 0 0 3.74.477m.94-3.197a5.971 5.971 0 0 0-.94 3.197M15 6.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm6 3a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Zm-13.5 0a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Z" />
                            </svg>
                            <Tooltip text="Volunteer to help others" />
                        </button>
                        ...
                        <button
                            onClick={async () => {
                                if (!userId) return;
                                const { data } = await supabase.from("question_notes").select(`id, help_others, profiles:user_id(name, avatar_url)`).eq("question_id", q.id).not("help_others", "eq", "none");
                                const helpers = data?.map((d: any) => ({ id: d.id, whatsapp: d.help_others, profiles: d.profiles })) || [];
                                setHelpMeHelpers(helpers);
                                setCurrentQuestionText(q.question_text);
                                setHelpMeOverlayOpen(true);
                            }}
                            className="group relative p-2 rounded-lg text-gray-400 hover:bg-green-50 hover:text-green-500 dark:hover:bg-green-900/20 transition-all"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                            ...
                            <Tooltip text="Find a study buddy" />
                        </button>
                        ...
                        <button
                            ...
                            className={`ml-2 group relative p-2.5 rounded-lg transition-all ${saved ? 'bg-green-500 text-white' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-md shadow-blue-200 dark:shadow-none'}`}
                        >
                            {saving ? (
                            ...
                            <Tooltip text={saving ? "Syncing..." : saved ? "Changes Saved" : "Save and Sync"} />
                        </button>
                    </div>
                </div>
            </div>
...
// Minimal Tooltip Sub-component
function Tooltip({ text }: { text: string }) {
    return (
        <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 pointer-events-none bg-gray-900 text-white text-[10px] px-2 py-1 rounded whitespace-nowrap transition-all duration-200 shadow-lg z-50">
            {text}
        </span>
    );
}