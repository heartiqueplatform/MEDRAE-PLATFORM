...
export function Announcements() {
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50/50 dark:bg-background pb-16 md:pb-10">
      <div className="w-full md:max-w-4xl md:mx-auto md:px-4 md:pt-6 space-y-4 md:space-y-6">
        ...
        <div className="px-4 md:px-0 pt-4 md:pt-0">
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            ...
            {unreadCount > 0 && (
              <Badge className="px-2.5 py-1 shrink-0 border-0 bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 font-normal text-xs self-start">
                {unreadCount} unread
              </Badge>
            )}
          </div>
          ...
          <Tabs defaultValue="all" className="space-y-3 md:space-y-4">
            ...
            <TabsContent value="all" className="space-y-3 md:space-y-4 outline-none">
              ...
              <div className="space-y-3 md:space-y-4">
                {pinnedAnnouncements.length > 0 && (
                  <h3 className="text-xs font-medium text-slate-500 dark:text-slate-400 px-4 md:px-0 uppercase tracking-wider">
                    Recent
                  </h3>
                )}
                {regularAnnouncements.map((a) => (
                  <div key={a.id} className="px-4 md:px-0">
                    <AnnouncementCard
                      announcement={a}
                      markAsRead={markAsRead}
                      readAnnouncements={readAnnouncements}
