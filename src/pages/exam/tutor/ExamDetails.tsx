...
const TutorExamDetails = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-background p-4 md:p-8 transition-colors duration-300">
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-2">
                        <button
                            onClick={handleBack}
                            className="group flex items-center text-slate-500 dark:text-slate-400 text-sm font-bold hover:text-indigo-600 dark:hover:text-indigo-400 transition-all"
                        >
                ...
                <div className="relative">
                    ...
                    <div className="absolute -inset-1 bg-gradient-to-r from-indigo-300 to-purple-300 rounded-xl blur opacity-10 dark:opacity-20 pointer-events-none" />

                    <div className="relative bg-white/80 dark:bg-background border-0 rounded-2xl shadow-2xl overflow-hidden">

                        {/* Toolbar Area */}
                        <div className="px-6 py-4 border-b border-0 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
                            <div className="flex items-center gap-2">
                                <PenTool className="w-4 h-4 text-slate-400" />
                                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Official Instruction Editor</span>
                            </div>
                        ...
                        <form onSubmit={handleSaveInstruction} className="p-6 md:p-10 space-y-6">
...
2. Use only a blue or black pen...`}
                                    ...
                                    required
                                    rows={12}
                                    className="w-full p-8 rounded-xl bg-white dark:bg-background border-2 border-slate-100 dark:border-slate-700 focus:border-indigo-500 dark:focus:border-indigo-500 focus:ring-0 outline-none text-slate-700 dark:text-slate-200 text-lg leading-relaxed font-serif placeholder:font-sans placeholder:text-slate-400 resize-none transition-all shadow-inner"
                                />

                                {/* Floating "Smart" Badge */}
                            ...
                            <Button
                                type="submit"
                                disabled={loading}
                                className="w-full h-14 bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl shadow-indigo-200 dark:shadow-none transition-all active:scale-[0.98] disabled:opacity-50 group"
                            >
                                {loading ? (
                ...
                {/* FOOTER DETAIL */}
                <p className="text-center text-[10px] text-slate-400 dark:text-slate-600 font-bold uppercase tracking-[0.3em]">
