...
import { Heart, Phone, CheckCircle, Trash2, Loader2, ImageIcon, Tag, Plus, Clock, } from "lucide-react";
...
const getExpiryInfo = (expiresAt: string | null) => {
    ...
    if (hours > 0) {
        return { label: `${hours}h ${minutes}m left`, urgent: true, expired: false };
    }
...
const ListingCardSkeleton = () => {
    return (
        <div className="group bg-white dark:bg-gray-900 rounded-2xl overflow-hidden md:shadow-sm animate-pulse">
            ...
            <div className="p-4 md:p-5">
                ...
                <div className="flex items-center gap-1.5 md:gap-2 mb-3 md:mb-4 p-1.5 md:p-2 bg-gray-50 dark:bg-gray-800 rounded-lg md:rounded-xl">
                ...
                <div className="grid grid-cols-4 gap-1 md:gap-2 py-2 md:py-3">
                    {[...Array(4)].map((_, i) => (
                ...
                <div className="flex gap-1.5 md:gap-2 mt-3 md:mt-4 pt-3 md:pt-4">
...
const PageSkeleton = () => {
    return (
        <div className="max-w-5xl mx-auto py-4 md:py-6 px-3 md:px-4 animate-pulse">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-2 pb-3 md:pb-0">
                ...
                <div className="flex flex-wrap items-center gap-2 md:gap-3">
                    <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg md:rounded-xl">
            ...
            <div className="mt-12 md:mt-20 py-8 md:py-10 text-center px-4 md:px-0">
...
export default function MarketFeed({ user }: any) {
    ...
    const fetchListings = async () => {
        ...
        // Hide expired listings permanently
    ...
    return (
        <div className="max-w-5xl mx-auto py-4 md:py-6 px-3 md:px-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-2 pb-3 md:pb-0">
                ...
                <div className="flex flex-wrap items-center gap-2 md:gap-3">
                    {/* Filter Dropdowns */}
                    <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg md:rounded-xl flex-1 md:flex-none">
                        <select
                    ...
                    <button
                        ...
                        className="flex items-center gap-1 md:gap-2 px-3 md:px-5 py-2 md:py-2.5 bg-blue-600 text-white font-bold rounded-lg md:rounded-xl hover:bg-blue-700 transition-all active:scale-95 text-[10px] md:text-sm"
                    >
                        <Plus size={16} className="md:w-5 md:h-5" /> <span className="hidden xs:inline">Sell</span> Item
                    </button>
                    <button
                        onClick={() => navigate("/market/my-listings")}
                        className="px-3 md:px-5 py-2 md:py-2.5 bg-gray-100 dark:bg-gray-800 rounded-lg md:rounded-xl font-bold text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-700 transition-all text-[10px] md:text-sm"
                    >
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 md:py-32 bg-gray-50 dark:bg-gray-900/50 rounded-2xl mt-4 md:mt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 lg:gap-6 mt-4 md:mt-6">
                    {listings.map((l) => {
                        ...
                        const expiry = getExpiryInfo(l.expires_at);

                        return (
                            <div
                                key={l.id}
                                className="group bg-white dark:bg-gray-900 rounded-2xl overflow-hidden transition-all duration-300 md:hover:shadow-2xl"
                            >
                                {/* Image Container */}
                                <div className="relative h-48 md:h-56 w-full overflow-hidden">
                                    ...
                                    <div className="absolute top-2 md:top-3 left-2 md:left-3 flex flex-col gap-1.5 md:gap-2">
                                        {l.is_featured && (
                                            <span className="bg-yellow-400 text-black text-[8px] md:text-[10px] font-black px-2 md:px-2.5 py-0.5 md:py-1 rounded-lg uppercase">
                                                Featured
                                            </span>
                                        )}
                                        {l.status === "sold" && (
                                            <span className="bg-red-600 text-white text-[8px] md:text-[10px] font-black px-2 md:px-2.5 py-0.5 md:py-1 rounded-lg uppercase">
                                                Sold Out
                                            </span>
                                        )}
                                    </div>
                                    ...
                                    {expiry && !expiry.expired && (
                                        <div className="absolute top-2 md:top-3 right-2 md:right-3">
                                            <span
                                                className={`flex items-center gap-1 backdrop-blur-md text-[8px] md:text-[10px] font-bold px-2 md:px-2.5 py-0.5 md:py-1 rounded-full uppercase ${expiry.urgent
                                    ...
                                    <div className="absolute bottom-2 md:bottom-3 right-2 md:right-3">
                                        <span className="bg-black/60 backdrop-blur-md text-white text-[8px] md:text-[10px] font-bold px-2 md:px-3 py-1 md:py-1.5 rounded-full uppercase">
                                            {l.category}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="p-4 md:p-5">
                                    ...
                                    {/* Dashboard Stats */}
                                    <div className="grid grid-cols-4 gap-1 md:gap-2 py-2 md:py-3 text-[8px] md:text-[10px] font-bold text-gray-400 uppercase tracking-tighter">
                                        ...
                                        {!isOwner && (
                                            <>
                                                <button
                                                    ...
                                                    className={`flex-1 flex items-center justify-center gap-1.5 md:gap-2 px-2 md:px-3 py-2 md:py-2.5 rounded-lg md:rounded-xl font-bold transition-all text-[9px] md:text-sm ${isSaved
                                                ...
                                                <button
                                                    ...
                                                    className="flex-1 flex items-center justify-center gap-1.5 md:gap-2 px-2 md:px-3 py-2 md:py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-lg md:rounded-xl font-bold transition-all text-[9px] md:text-sm"
                                                >
                                        ...
                                        {isOwner && (
                                            <>
                                                <button
                                                    ...
                                                    className={`flex-1 flex items-center justify-center gap-1.5 md:gap-2 px-2 md:px-3 py-2 md:py-2.5 rounded-lg md:rounded-xl font-bold text-white transition-all text-[9px] md:text-sm ${l.status === "sold" ? "bg-gray-500" : "bg-blue-600 hover:bg-blue-700"
                                                        }`}
                                                >
