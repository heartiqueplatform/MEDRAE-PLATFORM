...
const ExamBuddiesPage = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-background font-sans">
            ...
            <div className="sticky -top-4 z-30 bg-white dark:bg-slate-900 border-b border-slate-200/50 dark:border-slate-800/50">
                <div className="flex items-center gap-3 px-3 py-3 md:px-6 md:py-4 max-w-2xl mx-auto">
                    <button
                        onClick={() => navigate(-1)}
                        className="p-1.5 md:p-2 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 transition-transform active:scale-90"
                    >
                        <ChevronLeft size={20} className="md:w-5 md:h-5" />
                    </button>
                    <div className="flex-1 min-w-0">
                        <h1 className="text-lg md:text-xl font-heavy dark:text-white truncate tracking-tight">Exam Buddies</h1>
            ...
            <div className="px-0 md:px-4 lg:px-6 max-w-2xl mx-auto py-0 md:py-4 space-y-0 md:space-y-4">

                {/* Status Section - Mobile Native Style */}
                <div className="bg-white dark:bg-muted/30 px-4 py-6 md:p-8 md:rounded-xl border-b md:border md:border-slate-200/60 dark:md:border-slate-800 shadow-none md:shadow-sm">
                    <div className="relative z-10 flex flex-col items-center text-center">
                        <div className="h-14 w-14 md:h-16 md:w-16 bg-blue-50 dark:bg-blue-500/10 rounded-2xl md:rounded-3xl flex items-center justify-center text-blue-600 mb-3 md:mb-4">
                            <Users size={28} className="md:w-8 md:h-8" />
                        </div>
                        <h2 className="text-lg md:text-xl font-black dark:text-white mb-1">Sitting here in {currentCycle}?</h2>
                        <p className="text-sm text-slate-500 dark:text-slate-400 mb-5 md:mb-6">Join the list so others can connect with you for study or logistics.</p>

                        {!userGlobalStatus ? (
                            // CASE 1: Not joined anywhere yet
                            <button
                                onClick={() => setShowJoinModal(true)}
                                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 md:py-4 rounded-xl md:rounded-2xl flex items-center justify-center gap-2 transition-all active:scale-95"
                            >
                            ...
                            <button
                                onClick={() => setShowJoinModal(true)}
                                className="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-3.5 md:py-4 rounded-xl md:rounded-2xl flex items-center justify-center gap-2 transition-all active:scale-95"
                            >
                ...
                {/* Smart WhatsApp Group Banner - Mobile Native */}
                <div className="relative group overflow-hidden px-4 py-6 md:p-8 shadow-2xl shadow-emerald-200/50 dark:shadow-none border-b md:border md:border-emerald-400/20 md:rounded-xl">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-4 md:gap-6">
                        ...
                        <div className="flex-1 text-center md:text-left">
                            ...
                            <h3 className="text-xl md:text-2xl font-black text-white leading-tight mb-2 tracking-tighter">
                        ...
                        <div className="flex flex-col items-center gap-3 md:gap-4 min-w-[140px] md:min-w-[160px]">
                            {/* High-End WhatsApp Icon */}
                            <div className="relative">
                                <div className="absolute inset-0 bg-white/20 blur-xl rounded-full animate-pulse" />
                                <div className="relative h-14 w-14 md:h-16 md:w-16 bg-white rounded-2xl md:rounded-3xl flex items-center justify-center shadow-xl rotate-3 group-hover:rotate-6 transition-transform">
                                    <svg viewBox="0 0 24 24" className="w-8 h-8 md:w-10 md:h-10 fill-emerald-500">
                            ...
                            <a
                                ...
                                className="w-full bg-white text-emerald-700 hover:bg-emerald-50 px-5 md:px-6 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest text-center transition-all shadow-lg active:scale-95"
                            >
                                Join Now
                            </a>
                        </div>
                ...
                {/* Candidates List - Mobile Feed Style */}
                <div className="space-y-0 md:space-y-3">
                    <div className="flex items-center justify-between px-3 md:px-2 py-3 md:py-0">
                        <h3 className="font-black text-slate-800 dark:text-slate-200 flex items-center gap-2 text-base md:text-lg">
                            Candidates
                            <span className="text-blue-600 bg-blue-50 dark:bg-blue-900/30 px-2.5 md:px-3 py-0.5 rounded-full text-[10px] md:text-xs">{buddies.length}</span>
                        </h3>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="grid gap-0 md:gap-4">
                            {buddies.map((buddy, index) => (
                                <div key={buddy.id}>
                                    <div className="bg-white dark:bg-muted/30 px-3 py-3.5 md:p-4 md:rounded-[1.5rem] md:border md:border-slate-100 dark:md:border-slate-800/50 flex items-center gap-3 md:gap-4 transition-all hover:md:shadow-md">
                                        <div className="relative">
                                            <img
                                                src={buddy.student?.avatar_url || `https://ui-avatars.com/api/?name=${buddy.student?.name}&background=random`}
                                                className="h-12 w-12 md:h-14 md:w-14 rounded-xl md:rounded-2xl object-cover"
                                                alt=""
                                            />
                                            <div className="absolute -bottom-1 -right-1 h-4 w-4 md:h-5 md:w-5 bg-emerald-500 border-2 md:border-4 border-white dark:border-slate-900 rounded-full"></div>
                                        </div>

                                        <div className="flex-1 min-w-0">
                                            <h4 className="font-bold text-slate-900 dark:text-white text-sm md:text-base truncate">{buddy.student?.name}</h4>
                                            <div className="flex items-center gap-1 text-slate-500">
                                                <GraduationCap size={11} className="md:w-3 md:h-3" />
                                                <p className="text-[9px] md:text-[10px] font-bold uppercase tracking-tight truncate">
                                            ...
                                            <div className="flex gap-1.5 mt-1.5 md:mt-2">
                                                {buddy.is_looking_for_roommate && (
                                                    <span className="px-1.5 md:px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-[8px] md:text-[9px] font-black uppercase rounded flex md:rounded-md items-center gap-0.5 md:gap-1">
                                                        <BedDouble size={9} className="md:w-2.5 md:h-2.5" /> Room
                                                    </span>
                                                )}
                                                {buddy.is_looking_for_study_partner && (
                                                    <span className="px-1.5 md:px-2 py-0.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 text-[8px] md:text-[9px] font-black uppercase rounded flex md:rounded-md items-center gap-0.5 md:gap-1">
                                                        <BookOpen size={9} className="md:w-2.5 md:h-2.5" /> Study
                                                    </span>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <a
                                            ...
                                            target="_blank"
                                            className="h-10 w-10 md:h-12 md:w-12 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 rounded-xl md:rounded-2xl flex items-center justify-center transition-all hover:md:bg-emerald-500 hover:md:text-white active:scale-90"
                                        >
                                            <MessageSquare size={17} className="md:w-5 md:h-5" />
                                        </a>
                                    </div>
                        ...
                        <div className="text-center py-12 md:py-16 bg-slate-100/50 dark:bg-slate-900/50 rounded-2xl md:rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-slate-800 mx-3 md:mx-0">
            ...
            {/* JOIN MODAL - Keep original styling */}
            {showJoinModal && (
